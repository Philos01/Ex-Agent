...
- [图结构知识库](./graph-knowledge-base.md)
...
10. **LLM 供应商选择**：统一 LLM 客户端，支持 OpenAI、DeepSeek、Ollama，前端一键切换
11. **图结构知识库**：自动从文档中提取实体和关系，构建可交互可视化的知识图谱
12. **技能系统**：LLM 自主决策技能调用，支持 ArXiv 论文搜索、高德天气、网页搜索等
13. **速率限制**：内置登录和 API 速率限制，保护系统安全
14. **思考步骤显示**：支持启用思考步骤显示，便于理解 AI 推理过程
15. **ReAct 多步推理**：支持启用 ReAct 模式进行复杂多步推理
...
│   │   │   ├── auth.py         # 认证服务
│   │   │   ├── ingest.py       # 文档处理服务
│   │   │   ├── graph_store.py  # 图存储 (SQLite + NetworkX)
│   │   │   ├── graph_search.py # 图检索
│   │   │   ├── graph_analysis.py # 图分析 + 可视化
