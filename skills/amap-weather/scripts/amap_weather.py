...
def main():
    ...
    parser.add_argument("city", nargs="?", help="城市名（如 北京）或 adcode（如 110000）")
    ...
    parser.add_argument("--params", help="JSON 参数文件路径（用于技能系统调用）")
    ...
    if not city:
        print("错误: 未指定城市", file=sys.stderr)
        sys.exit(1)
    ...
    if json_output or is_skill_call:
        ...
        if data.get("status") != "1":
            result["error"] = data.get("info", "未知错误")
