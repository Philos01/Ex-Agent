...
Yufu Bai, Minchao Luo, Shenfu Zhang, Qiang Liu<sup>ID</sup>, Liang Chen<sup>ID</sup>, Weiwei Sun<sup>ID</sup>, *Senior Member, IEEE*, and Xiangchao Meng<sup>ID</sup>, *Senior Member, IEEE*
...
Yufu Bai, Minchao Luo, Shenfu Zhang, Liang Chen, and Xiangchao Meng are with the Faculty of Electrical Engineering and Computer Science, Ningbo University, Ningbo 315211, China (e-mail: baiyufu2001@126.com; 2311100052@nbu.edu.cn; zhangshenfu\_nbu@163.com; chenliang4@nbu.edu.cn; mengxiangchao@nbu.edu.cn).
...
![Figure 1: Schematic of the existing PAN-MS-HS fusion methods. (a) Step-by-step fusion: MR MS and LR HS are fused via MS-HS Fusion to produce MR HS. HR PAN and MR HS are then fused via PAN-HS Fusion to produce the final HR HS. (b) Information injection-based fusion: MR MS and HR PAN are processed through MS Spatial Information Extraction and PAN Spatial Information Extraction respectively. The extracted information is injected into the LR HS via Fusion Reconstruction to produce the final HR HS. (c) Feature concatenation-based fusion: MR MS, LR HS, and HR PAN are concatenated via Feature Concatenation and then processed through Reconstruction to produce the final HR HS.](aa9e46d6f962be5cebcbb5c654c9b13e_img.jpg)

Figure 1: Schematic of the existing PAN-MS-HS fusion methods. (a) Step-by-step fusion: MR MS and LR HS are fused via MS-HS Fusion to produce MR HS. HR PAN and MR HS are then fused via PAN-HS Fusion to produce the final HR HS. (b) Information injection-based fusion: MR MS and HR PAN are processed through MS Spatial Information Extraction and PAN Spatial Information Extraction respectively. The extracted information is injected into the LR HS via Fusion Reconstruction to produce the final HR HS. (c) Feature concatenation-based fusion: MR MS, LR HS, and HR PAN are concatenated via Feature Concatenation and then processed through Reconstruction to produce the final HR HS.
...
$$\text{CBRE}(\mathbf{F}_{in}) = \text{ReLU}(\text{BN}(\text{Conv}_{3 \times 3}(\mathbf{F}_{in})))) \quad (1)$$

![Overall architecture diagram of the proposed method. The diagram shows three input paths: LR HS (Low Resolution Hyperspectral), MR MS (Medium Resolution Multispectral), and HR PAN (High Resolution Panchromatic). LR HS is processed by FE-H to produce F_hs^0, which is then fused with MR MS (processed by FE-M to produce F_ms^0) via CAFB-1 to produce F_sw^1. HR PAN is processed by FE-P to produce F_pan^0, which is fused with F_sw^1 via CAFB-2 to produce F_M. F_M is then processed by SSRM-1 to produce F_R^0, which is fused with F_hs^0 via SSRM-2 to produce F_R^1. F_R^1 is processed by RE to produce the final HR HS output. SPAE modules are used for spatial feature enhancement. A legend at the bottom defines the modules: FE-H (Primary feature extraction for HS), FE-M (Primary feature extraction for MS), FE-P (Primary feature extraction for PAN), CAFB (Context-aware fusion block), SSRM (Spatial-spectral recombination module), SPAE (Spatial feature enhancement), and RE (Reconstruction module).](e394c2b5c61344f6a12397f430086072_img.jpg)

Overall architecture diagram of the proposed method. The diagram shows three input paths: LR HS (Low Resolution Hyperspectral), MR MS (Medium Resolution Multispectral), and HR PAN (High Resolution Panchromatic). LR HS is processed by FE-H to produce F\_hs^0, which is then fused with MR MS (processed by FE-M to produce F\_ms^0) via CAFB-1 to produce F\_sw^1. HR PAN is processed by FE-P to produce F\_pan^0, which is fused with F\_sw^1 via CAFB-2 to produce F\_M. F\_M is then processed by SSRM-1 to produce F\_R^0, which is fused with F\_hs^0 via SSRM-2 to produce F\_R^1. F\_R^1 is processed by RE to produce the final HR HS output. SPAE modules are used for spatial feature enhancement. A legend at the bottom defines the modules: FE-H (Primary feature extraction for HS), FE-M (Primary feature extraction for MS), FE-P (Primary feature extraction for PAN), CAFB (Context-aware fusion block), SSRM (Spatial-spectral recombination module), SPAE (Spatial feature enhancement), and RE (Reconstruction module).

Fig. 2. Overall architecture diagram of the proposed method.

$$\text{Conv}_{\text{res}}(\mathbf{F}_{\text{in}}) = \text{ReLU}(\text{BN}(\text{Conv}_{3 \times 3}(\text{CBRE}(\mathbf{F}_{\text{in}})))) + \mathbf{F}_{\text{in}} \quad (2)$$

$$\mathbf{F}_{\text{hs}}^0 = \text{CBRE}(\text{Up}(\text{Conv}_{1 \times 1}(\mathbf{H}))) \quad (3)$$

$$\mathbf{F}_{\text{ms}}^0 = \text{Up}(\text{Conv}_{3 \times 3}(\text{ReLU}(\text{Conv}_{3 \times 3}(\text{Conv}_{\text{res}}(\mathbf{M})))))) \quad (4)$$

$$\mathbf{F}_{\text{pan}}^0 = \text{CBRE}(\text{Conv}_{\text{res}}(\mathbf{P})) \quad (5)$$

where  $\text{CBRE}(\cdot)$  represents the sequential combination of a convolution layer with a  $3 \times 3$  kernel, a batch normalization layer, and an ReLU activation function.  $\text{Conv}_{\text{res}}(\cdot)$  refers to a basic structure in ResNet.  $\text{Conv}_{1 \times 1}(\cdot)$  denotes a convolution layer with a  $1 \times 1$  kernel, and  $\text{Up}(\cdot)$  indicates the upsampling operation.
...
The main component of this module is based on the cross-attention mechanism with windows and shifted windows. In the Swin Transformer, for a given feature  $\mathbf{F} \in \mathbb{R}^{H \times W \times C}$ , the feature is first divided using an  $M \times M$  window, splitting

it into  $HW/M^2$  nonoverlapping patches, each of shape  $M \times M \times C$ , which are then flattened into  $M^2 \times C$ , where  $HW/M^2$  represents the total number of windows. Next, standard self-attention operations are performed within each window. For the feature  $\mathbf{X} \in \mathbb{R}^{M^2 \times C}$  within a window, three learnable weight matrices,  $\mathbf{W}^Q \in \mathbb{R}^{C \times C}$ ,  $\mathbf{W}^K \in \mathbb{R}^{C \times C}$ , and  $\mathbf{W}^V \in \mathbb{R}^{C \times C}$ , are shared across windows to map the features into query, key, and value matrices, namely,  $\mathbf{Q}$ ,  $\mathbf{K}$ , and  $\mathbf{V}$

$$\mathbf{Q} = \mathbf{X}\mathbf{W}^Q, \mathbf{K} = \mathbf{X}\mathbf{W}^K, \mathbf{V} = \mathbf{X}\mathbf{W}^V. \quad (6)$$
...
$$\text{Attention} = \text{softmax}(\mathbf{Q}\mathbf{K}^T / \sqrt{D} + \beta) \mathbf{V} \quad (7)$$

where  $D$  is the dimensionality of  $\mathbf{Q}$  and  $\mathbf{K}$ , and  $\beta$  is the learnable relative position encoding.

Building upon this, we introduce cross-attention, where two input features,  $\mathbf{F}_1 \in \mathbb{R}^{H \times W \times C}$  and  $\mathbf{F}_2 \in \mathbb{R}^{H \times W \times C}$ , are simultaneously divided into nonoverlapping subwindows using an  $M \times M$  window. For the features  $\mathbf{X}_1 \in \mathbb{R}^{M^2 \times C}$  and  $\mathbf{X}_2 \in \mathbb{R}^{M^2 \times C}$  within each window,  $\mathbf{X}_1$  is used to map to the key and value matrices via learnable weight matrices, while  $\mathbf{X}_2$  is mapped to the query matrix. Finally, the attention results are computed uniformly. This process can be expressed as

$$\mathbf{Q}_2 = \mathbf{X}_2 \mathbf{W}^Q, \mathbf{K}_1 = \mathbf{X}_1 \mathbf{W}^K, \mathbf{V}_1 = \mathbf{X}_1 \mathbf{W}^V \quad (8)$$

$$\text{Attention} = \text{softmax}(\mathbf{Q}_2 \mathbf{K}_1^T / \sqrt{D} + \beta) \mathbf{V}_1. \quad (9)$$
...
![Figure 3: Schematic of CAFB. (a) CAFB. (b) W-MCA. (a) shows the flow of features F_hs^0 and F_ms^0 through Conv3D and Conv layers, followed by W-MCA and SW-MCA blocks with Norm and MLP layers. (b) shows the internal structure of W-MCA, including window splitting, cross-attention heads (Cross attention-h, Cross attention-1), and window reversal. A legend at the bottom defines the symbols: Conv3D (3D convolution operator), Conv (Convolution operator), MLP (Multi-Layer Perceptron), Linear (Linear layer), Norm (Layer normalization), W-MCA (Window multi-head cross attention), SW-MCA (Shifted window multi-head cross attention), R (Reshape), + (Element-wise addition), and x (Matrix multiplication).](a7d78d22e465dea388b31d0739f9d0cd_img.jpg)

Figure 3: Schematic of CAFB. (a) CAFB. (b) W-MCA. (a) shows the flow of features F\_hs^0 and F\_ms^0 through Conv3D and Conv layers, followed by W-MCA and SW-MCA blocks with Norm and MLP layers. (b) shows the internal structure of W-MCA, including window splitting, cross-attention heads (Cross attention-h, Cross attention-1), and window reversal. A legend at the bottom defines the symbols: Conv3D (3D convolution operator), Conv (Convolution operator), MLP (Multi-Layer Perceptron), Linear (Linear layer), Norm (Layer normalization), W-MCA (Window multi-head cross attention), SW-MCA (Shifted window multi-head cross attention), R (Reshape), + (Element-wise addition), and x (Matrix multiplication).

Fig. 3. Schematic of CAFB. (a) CAFB. (b) W-MCA.

![Figure 4: Illustration of the shifted window mechanism for computing attention in the Swin Transformer. The figure shows a 2x2 grid of images on the left, with a red arrow pointing to a 3x3 grid of images on the right. The 3x3 grid is divided into four quadrants, each containing a 2x2 sub-grid. The quadrants are labeled 1, 2, 3, 4. The 2x2 sub-grids are shifted by one pixel in each direction (up, down, left, right) to illustrate the shifted window mechanism.](bdc6095967437c168a3f2a4ff8ca38bd_img.jpg)
...
$$\begin{cases} \mathbf{F}_w^0 = W - MCA(\mathbf{F}_{hs}^1, \mathbf{F}_{ms}^1) + \mathbf{F}_{hs}^0 \\ \mathbf{F}_w^1 = MLP(LN(\mathbf{F}_w^0)) + \mathbf{F}_w^0 \end{cases} \quad (11)$$

$$\begin{cases} \mathbf{F}_{sw}^0 = SW - MCA(\mathbf{F}_w^1, \mathbf{F}_{ms}^0) + \mathbf{F}_w^1 \\ \mathbf{F}_{sw}^1 = MLP(LN(\mathbf{F}_{sw}^0)) + \mathbf{F}_{sw}^0 \end{cases} \quad (12)$$

where  $LN(\cdot)$  represents the layer normalization operation,  $MLP(\cdot)$  consists of two fully connected layers and a GELU activation function,  $W - MCA(\cdot)$  denotes window-based multi-head cross-attention (W-MCA), and  $SW - MCA(\cdot)$  represents shifted window multihead cross-attention. Meanwhile, the window size is 7 and the number of attention heads is 4.

The process of CAFB-2 is similar to that of CAFB-1. That is, the feature  $\mathbf{F}_{sw}^1$  output from CAFB-1, along with the primary feature  $\mathbf{F}_{pan}^0$ , is fed into CAFB-2 to obtain the intermediate feature  $\mathbf{F}_M$ .
...
- 1: Split  $\mathbf{F}_{hs}^1$ ,  $\mathbf{F}_{ms}^1$  into  $M \times M$  non-overlapping windows
...
- 4:    $\text{Attention} = \text{softmax}(\mathbf{Q}_2 \mathbf{K}_1^T / \sqrt{D} + \beta) \mathbf{V}_1$
- 5: **end for**
- 6:  $\mathbf{F}_{out} = \text{Window reverse}(\text{Attention}_1, \dots, \text{Attention}_h)$
- 7: **return**  $\mathbf{F}_{out} + \mathbf{F}_{hs}^0$

---

### C. Spatial-Spectral Feature Recombination Module

After the above process, although the intermediate feature  $\mathbf{F}_M$  contains multimodal information after passing through the

![Figure 5: Architecture of the spatial-spectral feature recombination module. (a) SSRM. (b) SPAE. (c) DSAM. The diagram shows three modules: (a) SSRM, (b) SPAE, and (c) DSAM. (a) SSRM takes F_M and F_ms^0 as input, processes them through DSAM and CAFBL. (b) SPAE takes F_pan^0 as input, processes it through Maxpool, Avgpool, Conv, Sigmoid, and element-wise multiplication. (c) DSAM takes F_M and F_ms^0 as input, processes them through Conv1, Resblock, Conv, GAP, FC, Softmax, and element-wise multiplication to produce F_D. A legend at the bottom defines the symbols: Maxpool (Max pooling), Avgpool (Average pooling), Conv1 (1x1 convolution), GAP (Global Average Pooling), Resblock (Resnet block), FC (Fully connected layer), Concatenation (C), and Element-wise multiplication (dot).](a738993919a50143787084ee7ce6e2f2_img.jpg)

Figure 5: Architecture of the spatial-spectral feature recombination module. (a) SSRM. (b) SPAE. (c) DSAM. The diagram shows three modules: (a) SSRM, (b) SPAE, and (c) DSAM. (a) SSRM takes F\_M and F\_ms^0 as input, processes them through DSAM and CAFBL. (b) SPAE takes F\_pan^0 as input, processes it through Maxpool, Avgpool, Conv, Sigmoid, and element-wise multiplication. (c) DSAM takes F\_M and F\_ms^0 as input, processes them through Conv1, Resblock, Conv, GAP, FC, Softmax, and element-wise multiplication to produce F\_D. A legend at the bottom defines the symbols: Maxpool (Max pooling), Avgpool (Average pooling), Conv1 (1x1 convolution), GAP (Global Average Pooling), Resblock (Resnet block), FC (Fully connected layer), Concatenation (C), and Element-wise multiplication (dot).
...
As shown in Fig. 5,  $\mathbf{F}_M$  and  $\mathbf{F}_{ms}^0$  are aggregated through a residual structure to form the upper branch's features. Then,  $\mathbf{F}_{ms}^0$  is separately processed through a convolution to form the second branch's features. The features are then aggregated through an elementwise summation. Afterward, a global average pooling operation is applied to the summed feature map along the spatial dimension to generate a channel vector. This vector is fed into a fully connected layer to compress the channels, and then, the channels are restored through two separate fully connected layers, generating two channel vectors. Finally, the softmax function is applied to produce complementary selective weights, which are applied to recalibrate and reassemble the two input branches, dynamically adjusting and reassembling

the spectral features

$$\begin{cases} \mathbf{F}_d^1 = \text{Conv}_{3 \times 3} (\text{Conv}_{\text{res}} (\text{Cat} (\mathbf{F}_M, \mathbf{F}_{ms}^0))) \\ \mathbf{F}_d^2 = \text{Conv}_{1 \times 1} (\mathbf{F}_{ms}^0) \end{cases} \quad (13)$$

$$\begin{cases} \mathbf{W}^1 = \text{FC}_1 (\text{FC} (\text{GAP} (\mathbf{F}_d^1 + \mathbf{F}_d^2))) \\ \mathbf{W}^2 = \text{FC}_2 (\text{FC} (\text{GAP} (\mathbf{F}_d^1 + \mathbf{F}_d^2))) \end{cases} \quad (14)$$

$$\begin{cases} \mathbf{W}_s^1, \mathbf{W}_s^2 = \text{softmax} (\mathbf{W}^1, \mathbf{W}^2) \\ \mathbf{W}_{s_i}^1 = \frac{e^{\mathbf{W}_i^1}}{e^{\mathbf{W}_i^1} + e^{\mathbf{W}_i^2}} \\ \mathbf{W}_{s_i}^2 = \frac{e^{\mathbf{W}_i^2}}{e^{\mathbf{W}_i^1} + e^{\mathbf{W}_i^2}} \end{cases} \quad (15)$$

$$\mathbf{F}_D = \mathbf{W}_s^1 * \mathbf{F}_d^1 + \mathbf{W}_s^2 * \mathbf{F}_d^2 \quad (16)$$

where the concatenation operation, global average pooling function, and fully connected layer are denoted by  $\text{Cat}(\cdot)$ ,  $\text{GAP}(\cdot)$ , and  $\text{FC}(\cdot)$ , respectively. The compression ratio of the FC layer is 16. The softmax operation is applied to the elements at the corresponding positions of vectors  $\mathbf{W}^1$  and  $\mathbf{W}^2$ .  $\mathbf{W}_{s_i}^1$  ( $\mathbf{W}_{s_i}^2$ ) and  $\mathbf{W}_i^1$  ( $\mathbf{W}_i^2$ ) represent the elements in vectors  $\mathbf{W}_s^1$  ( $\mathbf{W}_s^2$ ) and  $\mathbf{W}^1$  ( $\mathbf{W}^2$ ), respectively.  $\mathbf{W}_s^1$  and  $\mathbf{W}_s^2$  represent the weights of the two branch features.
...
![Diagram of the feature reconstruction module (RE). The module takes an input feature map F_R^1 and a hyperspectral (HS) image. F_R^1 is processed by a Conv layer, followed by BN and ReLU, and then another Conv layer. The output is added (+) to the HS image. The HS image is then upsampled (Up) to produce the final reconstructed feature map.](990567efebf979be51f56d1150012c9d_img.jpg)

Diagram of the feature reconstruction module (RE). The module takes an input feature map F\_R^1 and a hyperspectral (HS) image. F\_R^1 is processed by a Conv layer, followed by BN and ReLU, and then another Conv layer. The output is added (+) to the HS image. The HS image is then upsampled (Up) to produce the final reconstructed feature map.

Fig. 6. Illustration of feature reconstruction module.

First, a spatial feature enhancement module (SPAE), composed of pooling and self-guided forms, is used to extract spatial information from  $\mathbf{F}_{\text{pan}}^0$ . Then, the CAFBL maps the spatial information into  $\mathbf{Q}_2$ . This process adjusts and corrects the features after the DSAM, allowing the spatially refined features to be integrated

$$\mathbf{F}_{\text{spae}} = \text{sigmoid}(\text{Conv}_{3 \times 3}(\text{Pavg}(\mathbf{F}_{\text{pan}}^0) + \text{Pmax}(\mathbf{F}_{\text{pan}}^0))) \otimes \mathbf{F}_{\text{pan}}^0 \quad (17)$$

$$\mathbf{F}_R^0 = \text{CAFBL}(\mathbf{F}_{\text{spae}}, \mathbf{F}_D) \quad (18)$$

where  $\text{Pavg}(\cdot)$  and  $\text{Pmax}(\cdot)$  represent the average pooling operation and the max pooling operation, respectively.  $\otimes$  denotes the elementwise multiplication, and  $\text{CAFBL}(\cdot)$  represents the CAFB with preconvolution removed. The reason for using CAFBL instead of CAFB is that the input features (e.g.,  $\mathbf{F}_D$  and  $\mathbf{F}_{\text{pan}}^0$ ) have already been optimized by DSAM and SPAE for spectral calibration and spatial enhancement. Adding preconvolution layers here would make these refined features oversmoothed, especially the high-frequency edges in PAN. By directly feeding raw features into CAFBL, the cross-modal attention can better capture the intrinsic spatial-spectral relationships without interference from redundant filtering.

Following the above process, the feature  $\mathbf{F}_R^0$  after SSRM-1 and the shallow feature  $\mathbf{F}_{\text{hs}}^0$  are, then, fed into SSRM-2 together, resulting in the fused feature  $\mathbf{F}_R^1$  before the final fusion.

#### Algorithm 2 DSAM

- 
- Require:**  $\mathbf{F}_M, \mathbf{F}_{\text{ms}}^0$ , compression ratio  $r = 16$
- 1:  $\mathbf{F}_d^1 = \text{Conv}_{3 \times 3}(\text{Conv}_{\text{res}}(\text{Cat}(\mathbf{F}_M, \mathbf{F}_{\text{ms}}^0)))$
  - 2:  $\mathbf{F}_d^2 = \text{Conv}_{1 \times 1}(\mathbf{F}_{\text{ms}}^0)$
  - 3:  $\mathbf{U} = \text{GAP}(\mathbf{F}_d^1 + \mathbf{F}_d^2)$
  - 4:  $\mathbf{W}^1 = \text{FC}_1(\text{FC}(\mathbf{U})), \mathbf{W}^2 = \text{FC}_2(\text{FC}(\mathbf{U}))$
  - 5:  $\mathbf{W}_s^1, \mathbf{W}_s^2 = \text{softmax}(\mathbf{W}^1, \mathbf{W}^2)$
  - 6: **return**  $\mathbf{W}_s^1 \odot \mathbf{F}_d^1 + \mathbf{W}_s^2 \odot \mathbf{F}_d^2$
- 
...
$$\mathbf{F}_R = \text{Conv}_{3 \times 3}(\text{CBRE}(\mathbf{F}_R^1)) \quad (19)$$

$$\mathbf{Z} = \mathbf{F}_R + \text{Up}(\mathbf{H}) \quad (20)$$

where the operation of  $\text{CBRE}(\cdot)$  is shown in (1), and  $\text{Up}(\mathbf{H})$  represents the upsampled HS image.
...
$$L_1 = \frac{1}{N} \sum_{n=1}^N \|\mathbf{Z}' - \mathbf{Z}\|_1 \quad (21)$$
...
We evaluated the fusion performance of the proposed method with deep learning-based techniques like MGFEI [48], HMPNet [47], and HyperNet [45], as well as with other integrated PAN–MS–HS fusion methods that are currently in use, such as CNMF [37]. We also extended other deep learning-based methods—SMGU-Net [56], DCFNet [52], HyperPNN [18], and TFNet [53]—to support integrated PAN–MS–HS fusion. Additionally, we applied a sequential fusion strategy to the FUSE [16] and HySure [54] methods, where each method first fuses MS and HS images to obtain a preliminary HS image with enhanced spatial resolution. This result is, then, fused with the PAN image to achieve the final output. For DCFNet and TFNet, we added an HS image branch that mirrors the original structure. For HyperPNN, we incorporated an MS image branch. For SMGU-Net, we add a PAN image branch.
...
![Figure 7: Visual comparison of fusion results on Chikusei dataset. The figure is a 2x6 grid of images. Each row contains six sub-images labeled (a) through (f). Each sub-image shows a wide view of a scene with a red box highlighting a specific region, and a zoomed-in view of that region below it. The sub-images represent different fusion methods: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, and (l) Reference. The proposed method (k) shows superior spatial and spectral quality compared to the others.](349ca0a6a9c2e2651a4deeeaf8be6da1_img.jpg)

Figure 7: Visual comparison of fusion results on Chikusei dataset. The figure is a 2x6 grid of images. Each row contains six sub-images labeled (a) through (f). Each sub-image shows a wide view of a scene with a red box highlighting a specific region, and a zoomed-in view of that region below it. The sub-images represent different fusion methods: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, and (l) Reference. The proposed method (k) shows superior spatial and spectral quality compared to the others.
...
![Figure 8: Spectral error map of fusion results on Chikusei dataset. The figure is a 2x6 grid of heatmaps. Each row contains six sub-images labeled (a) through (f). Each sub-image shows a spectral error map for a different fusion method. A color bar on the right indicates the error scale from 0 (blue) to 50 (red). The proposed method (k) shows the lowest error, indicated by the predominantly blue color.](6b32b7b928d34eeccb15c29cdf9d2cb3_img.jpg)

Figure 8: Spectral error map of fusion results on Chikusei dataset. The figure is a 2x6 grid of heatmaps. Each row contains six sub-images labeled (a) through (f). Each sub-image shows a spectral error map for a different fusion method. A color bar on the right indicates the error scale from 0 (blue) to 50 (red). The proposed method (k) shows the lowest error, indicated by the predominantly blue color.
...
TABLE I  
COMPARISON OF QUANTITATIVE EVALUATION METRICS ON CHIKUSEI DATASET. BOLD AND UNDERLINE: BEST AND SECOND BEST, RESPECTIVELY

| Method   | PSNR $\uparrow$ | SSIM $\uparrow$ | SAM $\downarrow$ | ERGAS $\downarrow$ | RMSE $\downarrow$ | CC $\uparrow$ |
|----------|-----------------|-----------------|------------------|--------------------|-------------------|---------------|
...
| MGFEI    | <u>40.0321</u>  | <u>0.9876</u>   | <u>2.4539</u>    | <u>1.4018</u>      | <u>0.0101</u>     | <u>0.9845</u> |
| Proposed | <b>41.1152</b>  | <b>0.9897</b>   | <b>2.1845</b>    | <b>1.2437</b>      | <b>0.0089</b>     | <b>0.9873</b> |

![Figure 9: Visual comparison of fusion results on Pavia Center dataset. The figure is a 4x6 grid of images. The first row shows the original images with yellow boxes highlighting specific regions. The second row shows zoomed-in views of these regions for methods (a) through (f). The third row shows the original images with yellow boxes for methods (g) through (l). The fourth row shows zoomed-in views of these regions for methods (g) through (l). The methods are: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, and (l) Reference.](d29cfbf30a471dc06a78be27f86bd1cf_img.jpg)

Figure 9: Visual comparison of fusion results on Pavia Center dataset. The figure is a 4x6 grid of images. The first row shows the original images with yellow boxes highlighting specific regions. The second row shows zoomed-in views of these regions for methods (a) through (f). The third row shows the original images with yellow boxes for methods (g) through (l). The fourth row shows zoomed-in views of these regions for methods (g) through (l). The methods are: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, and (l) Reference.
...
Fig. 10 further validates the proposed method with the spectral angle error map, where the proposed method achieves the smallest error, indicating its fusion results are closest to the reference image. Table II summarizes the quantitative evaluation metrics. While HySure, FUSE, and CNMF underperform compared to deep learning-based methods, their performance is similar to the simpler HyperPNN method.

The proposed method outperforms all others across all metrics, demonstrating its superiority.
...
![Figure 10: Spectral error map of fusion results on Pavia Center dataset. The figure consists of 12 sub-images labeled (a) through (l). Sub-images (a) to (k) show the spectral error maps for various fusion methods: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, and (k) Proposed. Sub-image (l) is the Reference image. A color bar on the right indicates the error scale from 0 to 80, with blue representing low error and red representing high error.](f176174c2978785e86a8352bd45e322e_img.jpg)
...
TABLE II  
COMPARISON OF QUANTITATIVE EVALUATION METRICS ON PAVIA CENTER DATASET. BOLD AND UNDERLINE: BEST AND SECOND BEST, RESPECTIVELY
...
| MGFEI    | <u>37.3395</u>  | <u>0.9820</u>   | <u>4.2307</u>    | <u>2.2036</u>      | <u>0.0135</u>     | <u>0.9874</u> |
| Proposed | <b>38.0181</b>  | <b>0.9842</b>   | <b>3.9319</b>    | <b>2.0382</b>      | <b>0.0125</b>     | <b>0.9894</b> |

TABLE III  
COMPARISON OF QUANTITATIVE EVALUATION METRICS ON FULL-RESOLUTION HZB DATASET. BOLD AND UNDERLINE: BEST AND SECOND BEST, RESPECTIVELY

| Method         | HySure | FUSE   | CMNF   | HyperPNN | TFNet  | DCFNet | HyperNet | HMPNet | SMGU-Net | MGFEI         | Proposed      |
|----------------|--------|--------|--------|----------|--------|--------|----------|--------|----------|---------------|---------------|
| QNR $\uparrow$ | 0.6143 | 0.3933 | 0.8134 | 0.7537   | 0.8126 | 0.8416 | 0.8126   | 0.8425 | 0.8368   | <u>0.8441</u> | <b>0.8452</b> |
...
To assess spectral performance, Fig. 12 shows the comparison of the spectral curves (i.e., reflectance/radiance intensity variations across contiguous narrow bands) of three typical land features, unused land, buildings, and roads between different fusion methods and the reference image. These pixel-level curves represent discrete samplings of the materials' spectral fingerprints, where fluctuations encode their unique spectral response characteristics. The proposed method (red

curve) closely matches the reference HS image (black curve), demonstrating superior spectral fidelity in preserving these discriminative features.
...
In Fig. 13(a)–(c), like the HZB dataset, HySure shows severe blocky distortions, FUSE has significant spectral distortion with red artifacts, and CMNF suffers from missing spatial details, resulting in unclear features. In Fig. 13(d)–(f), HyperPNN exhibits the most severe distortions, especially abnormal red artifacts around buildings. TFNet is slightly clearer but still has noticeable noise, while DCFNet shows some improvement but remains spatially and spectrally deficient. In Fig. 13(g)–(k), HyperNet exhibits relatively high spatial blurring and spectral misalignment. HMPNet and SMGU-Net exhibit severe spectral and spatial distortions, respectively. While the MGFEI and proposed methods yield similar results, some deficiencies remain. For instance, in Fig. 13(h), building junction details

![Figure 11: Visual comparison of fusion results on full-resolution HZB dataset. The figure shows a 4x7 grid of images. The first row contains original images with red boxes highlighting specific areas. The subsequent three rows show zoomed-in views of these areas for different fusion methods: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, (l) PAN, (m) MS, and (n) HS. The proposed method (k) shows superior detail preservation and color accuracy compared to the others.](bb6d33498937738ff5dac8d91c9ebaad_img.jpg)

Figure 11: Visual comparison of fusion results on full-resolution HZB dataset. The figure shows a 4x7 grid of images. The first row contains original images with red boxes highlighting specific areas. The subsequent three rows show zoomed-in views of these areas for different fusion methods: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, (l) PAN, (m) MS, and (n) HS. The proposed method (k) shows superior detail preservation and color accuracy compared to the others.
...
![Figure 12: Comparison of single-pixel spectral values at three typical land features in the HZB dataset. The figure contains three line graphs: 'Unused land', 'Buildings', and 'Road'. Each graph plots 'Normalized Spectral Value' (y-axis) against 'Band Number' (x-axis, 0 to 140). The legend identifies the Reference (black solid line), Proposed (red solid line), MGFEI (blue dashed), SMGU-Net (green dashed), HyperNet (yellow dashed), HMPNet (cyan dashed), TFNet (light green dashed), DCFNet (grey dashed), HyperPNN (purple dashed), CNMF (pink dashed), FUSE (orange dashed), and HySure (red dashed). In all three graphs, the Proposed method's spectral curve closely follows the Reference curve, while other methods show significant deviations, especially in the 'Buildings' and 'Road' graphs.](e6b5ee67ac260b0a3ed3e3c5ad7ea19c_img.jpg)

Figure 12: Comparison of single-pixel spectral values at three typical land features in the HZB dataset. The figure contains three line graphs: 'Unused land', 'Buildings', and 'Road'. Each graph plots 'Normalized Spectral Value' (y-axis) against 'Band Number' (x-axis, 0 to 140). The legend identifies the Reference (black solid line), Proposed (red solid line), MGFEI (blue dashed), SMGU-Net (green dashed), HyperNet (yellow dashed), HMPNet (cyan dashed), TFNet (light green dashed), DCFNet (grey dashed), HyperPNN (purple dashed), CNMF (pink dashed), FUSE (orange dashed), and HySure (red dashed). In all three graphs, the Proposed method's spectral curve closely follows the Reference curve, while other methods show significant deviations, especially in the 'Buildings' and 'Road' graphs.

Fig. 12. Comparison of single-pixel spectral values at three typical land features in the HZB dataset.

TABLE IV  
COMPARISON OF QUANTITATIVE EVALUATION METRICS ON FULL-RESOLUTION YC DATASET. BOLD AND UNDERLINE: BEST AND SECOND BEST, RESPECTIVELY

| Method | HySure | FUSE   | CMNF   | HyperPNN | TFNet  | DCFNet | HyperNet | HMPNet | SMGU-Net | MGFEI         | Proposed      |
|--------|--------|--------|--------|----------|--------|--------|----------|--------|----------|---------------|---------------|
| QNR↑   | 0.7833 | 0.6407 | 0.7778 | 0.8038   | 0.7983 | 0.8148 | 0.8101   | 0.8144 | 0.8173   | <u>0.8200</u> | <b>0.8273</b> |
...
![Figure 13: Visual comparison of fusion results on full-resolution YC dataset. The figure shows 14 sub-images (a-n) arranged in a 2x7 grid. Each sub-image consists of a full-resolution aerial view and a corresponding zoomed-in rectangular patch. The methods compared are: (a) HySure, (b) FUSE, (c) CMNF, (d) HyperPNN, (e) TFNet, (f) DCFNet, (g) HyperNet, (h) HMPNet, (i) SMGU-Net, (j) MGFEI, (k) Proposed, (l) PAN, (m) MS, and (n) HS. Red boxes in the full-resolution images highlight specific features for comparison in the zoomed-in patches.](7832324609ad3cc688064e0341612b32_img.jpg)
...
![Figure 14: Comparison of single-pixel spectral values at three typical land features in the YC dataset. The figure contains three line graphs: 'Farmland', 'Buildings', and 'Water'. Each graph plots 'Normalized Spectral Value' (y-axis) against 'Band Number' (x-axis, 0 to 140). The legend identifies the following methods: Reference (black solid line), Proposed (red solid line), MGFEI (blue dashed line), SMGU-Net (green dashed line), HMPNet (cyan dashed line), HyperNet (yellow dashed line), DCFNet (grey dashed line), TFNet (light green dashed line), HyperPNN (pink dashed line), CNMF (purple dashed line), FUSE (orange dashed line), and HySure (red dashed line). The graphs show how each method's spectral output compares to the reference across different land cover types.](411fa16c3211377525ba37c57784fee0_img.jpg)

Figure 14: Comparison of single-pixel spectral values at three typical land features in the YC dataset. The figure contains three line graphs: 'Farmland', 'Buildings', and 'Water'. Each graph plots 'Normalized Spectral Value' (y-axis) against 'Band Number' (x-axis, 0 to 140). The legend identifies the following methods: Reference (black solid line), Proposed (red solid line), MGFEI (blue dashed line), SMGU-Net (green dashed line), HMPNet (cyan dashed line), HyperNet (yellow dashed line), DCFNet (grey dashed line), TFNet (light green dashed line), HyperPNN (pink dashed line), CNMF (purple dashed line), FUSE (orange dashed line), and HySure (red dashed line). The graphs show how each method's spectral output compares to the reference across different land cover types.
...
TABLE VI  
COMPARISON OF PARAMETERS, FLOPS, AND TESTING TIME FOR DIFFERENT METHODS ON FULL-RESOLUTION HZB DATASET

| Method    | HySure | FUSE | CMNF  | HyperPNN     | TFNet  | DCFNet      | HyperNet | HMPNet | SMGU-Net | MGFEI   | Proposed |
|-----------|--------|------|-------|--------------|--------|-------------|----------|--------|----------|---------|----------|
| Params(M) | /      | /    | /     | <b>0.14</b>  | 2.57   | 7.74        | 2.84     | 4.39   | 18.05    | 6.15    | 3.75     |
| Flops(G)  | /      | /    | /     | <b>32.60</b> | 141.97 | 191.37      | 653.61   | 1819   | 5819     | 1107.26 | 837.80   |
| Time(s)   | 293.87 | 7.67 | 27.68 | 2.68         | 2.37   | <b>2.28</b> | 3.16     | 2.63   | 3.10     | 2.45    | 2.91     |
...
- [8] T.-M. Tu, S.-C. Su, H.-C. Shyu, and P. S. Huang, "A new look at IHS-like image fusion methods," *Inf. Fusion*, vol. 2, no. 3, pp. 177–186, Sep. 2001.
...
- [24] M. Selva, B. Aiazzi, F. Butera, L. Chiarantini, and S. Baronti, "Hyper-sharpening: A first approach on SIM-GA data," *IEEE J. Sel. Topics Appl. Earth Observ. Remote Sens.*, vol. 8, no. 6, pp. 3008–3024, Jun. 2015.
...
- [32] W. He, Y. Chen, N. Yokoya, C. Li, and Q. Zhao, "Hyperspectral super-resolution via coupled tensor ring factorization," *Pattern Recognit.*, vol. 122, Feb. 2022, Art. no. 108280.
...
- [47] X. Tian, K. Li, W. Zhang, Z. Wang, and J. Ma, "Interpretable model-driven deep network for hyperspectral, multispectral, and panchromatic image fusion," *IEEE Trans. Neural Netw. Learn. Syst.*, vol. 35, no. 10, pp. 14382–14395, Oct. 2023.
...
- [54] M. Simões, J. Bioucas-Dias, L. B. Almeida, and J. Chanussot, "A convex formulation for hyperspectral image superresolution via subspace-based regularization," *IEEE Trans. Geosci. Remote Sens.*, vol. 53, no. 6, pp. 3373–3388, Jun. 2015.
...
![Portrait of Yufu Bai](6cc4a2d5ea0462e4825d57bd689bd2b3_img.jpg)

Portrait of Yufu Bai
...
![Portrait of Minchao Luo](c3ee41c6d46565ab4198d0a9c69108c5_img.jpg)
...
![Portrait of Shenfu Zhang](bcc4b9d57d1d23e256f09d0a0a81be73_img.jpg)
...
![Portrait of Qiang Liu](02e2819551a3fd5c0a5e941008b5c526_img.jpg)
...
![Portrait of Liang Chen](3d6d203581d8e268e252ca7f8476f829_img.jpg)
...
![Portrait of Weiwei Sun](c83a577f84ae898d1cd4f4057fd436ca_img.jpg)
...
He studied with the College of Electrical and Information Engineering, Hunan University, Changsha, Hunan, China, working as a Post-Doctoral Researcher. He is currently a Professor with the Faculty of Electrical Engineering and Computer Science, Ningbo University, Ningbo, China. His research interests include multisource data fusion and machine learning.